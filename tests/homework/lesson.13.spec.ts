...
test(`Lesson 13 Homework`, async ({ page }) => {
  ...
  await test.step('Radio Button Example', async () => {
    ...
    await expect.soft(page.locator('label').filter({ hasText: 'Radio2' }).getByRole('radio'), 'Verify Radio button #2 is checked').toBeChecked();
  });
  ...
  await test.step('Mouse Hover Example', async () => {
    await page.getByRole('button', { name: /Mouse Hover/i }).hover();
    await expect.soft(page.locator('a:has-text("Top")'), 'Verify the "Top" link is now visible').toBeVisible();
  });
});
