...
test(`Lesson 13 Homework`, async ({ page }) => {
  ...
  await test.step('Radio Button Example', async () => {
    ...
    await page.locator('//input[contains(@value, "radio2")]').check();
    // Verify Radio button #2 is checked
    await expect.soft(page.locator('//input[contains(@value, "radio2")]'), 'Verify Radio button #2 is checked').toBeChecked();
  });

  await test.step('Suggession Class Example', async () => {
    // fill the input with "Bulgaria"
    await page.locator('#autocomplete').fill('Bulgaria'); // alternative locator: page.getByPlaceholder('Type to Select Countries')
    // click on the first auto complete suggestion
    await page.locator('#ui-id-1').click();
    // Verify the input value is "Bulgaria"
    await expect.soft(page.locator('#autocomplete'), 'Verify Text is Bulgaria').toHaveValue('Bulgaria');
  });
  ...
  await test.step('Dropdown Example', async () => {
    ...
    await expect.soft(page.locator('#dropdown-class-example'), 'Verify Selected option is option2').toHaveValue('option2');
  });
  ...
  await test.step('Element Displayed Example', async () => {
    ...
    await expect.soft(page.locator('#displayed-text'), 'Verify Element is not visible!').toBeHidden(); // alternative locator: page.getByPlaceholder('Hide/Show Example')
  });

  await test.step('Mouse Hover Example', async () => {
    // hover over the "Mouse Hover" element
    await page.locator('#mousehover').hover(); // alternative locator: page.getByRole('button', { name: 'Mouse Hover' })
    // Verify the "Top" link is now visible
    await expect.soft(page.getByRole('link', { name: 'Top' }), 'Verify Element is visible').toBeVisible();
  });
});
